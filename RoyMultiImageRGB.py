...
def processImage(imgFilename,data):
	MIN_BEAD_AREA = 5
	CUT_OFF_AREA = 1500
	DARKNESS_THRESHOLD = 20 #out of 255
	...
	ret, thresh = cv2.threshold(img_max,DARKNESS_THRESHOLD,255,cv2.THRESH_BINARY)
	#ret, thresh = cv2.threshold(img_max,0,255,cv2.THRESH_BINARY+cv2.THRESH_OTSU)
	#thresh = cv2.adaptiveThreshold(img_gray,30,cv2.ADAPTIVE_THRESH_GAUSSIAN_C,cv2.THRESH_BINARY,5,2)
		...
		if cv2.contourArea(contour) > MIN_BEAD_AREA :
			...
			if area > CUT_OFF_AREA:
			 ...
			 if cx > PADDING and cx < (img.shape[1]-PADDING) and cy > PADDING and cy< (img.shape[0]-PADDING):
				 ...
				 imgcropsquare = img2[(cy -7 ):(cy + 7),(cx -7 ):(cx + 7) ,:]
	...
	if beadcount == 0:
		...
		#show image
		resizedfgcopy = cv2.resize(dist_transform,(500,400))
